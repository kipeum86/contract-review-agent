...
> Claude Code 기반의 AI 계약 검토 파이프라인입니다.
> 계약서를 넣으면 **추적 변경이 적용된 레드라인 Word 파일, 여백 코멘트(내부 전략용 + 외부 공유용), 전체 분석 보고서, 협상 권고안**을 모두 DOCX로 직접 생성해 돌려줍니다.
...
Contract Review Agent는 법률 계약 운영을 위한 **Claude Code 네이티브** 프로젝트입니다.
**VS Code 통합 터미널**의 `claude` CLI나 **Claude Code VS Code 확장 프로그램**의 채팅 패널에서 사용할 수 있으며, 두 방식 모두 같은 슬래시 명령과 자연어 입력을 지원합니다.

| 기능 | 설명 |
|------|------|
| **Ingest** | 사내 템플릿, 선례, 플레이북으로 검색 가능한 라이브러리를 구축합니다 |
| **Review** | 상대방 계약서를 사내 기준과 비교해 조항별로 분석합니다 |
| **Re-review** | 협상 후 수정본이 돌아왔을 때 변경분을 분석합니다 |
| **Draft** | 인터뷰 기반으로 계약서를 초안 작성하고 자체 검토합니다 *(로드맵)* |

모든 처리는 **로컬 파일시스템에서만** 이루어집니다. 외부 서버도 없고, 벡터 데이터베이스도 없으며, 데이터가 사용자의 컴퓨터를 벗어나지 않습니다.
...
Claude Code는 계약군, 조항 분류 체계, 검토 모드, 검색 규칙 등을 포함한 여섯 개 정책 파일을 한 번에 다시 작성할 수 있습니다. 필요하면 [YAML 파일을 직접 수정](#policy-files)해도 됩니다.

> **팁 — 아직 정책을 어떻게 잡아야 할지 모르겠다면?** 먼저 3단계로 가세요. 사내 템플릿을 ingest한 다음, 다시 돌아와 Claude Code에게 이미 ingest된 계약서를 기준으로 정책을 맞춰 달라고 하세요:
>
> ```text
...
| 구조 | 파일 하나당 계약서 하나 |
| 프라이버시 | 업로드한 파일은 로컬 PC에만 남고, 업로드되거나 공유되지 않습니다 |
...
Target contract (DOCX/PDF)
    │
    ├── 조항 단위로 파싱 및 분할
    ├── 라이브러리에서 대응하는 사내 조항 검색
    ├── 조항별 비교 수행 (리스크 등급, 갭 분석)
    ├── 레드라인 제안과 코멘트 생성
    ├── DOCX에 추적 변경 적용
    │
    ├── 📄 내부용 레드라인 DOCX   (모든 코멘트 포함)
...
| 모드 | 사용 시점 | 레드라인 범위 |
|------|-----------|---------------|
| **strict** | 고액 딜, M&A, 협상력이 강할 때 | 모든 이탈 사항 |
| **moderate** | 일반적인 상거래 계약 | Critical + High risk |
| **loose** | 협상력이 약하거나 빠른 검토가 필요한 경우, LOI/MOU | Critical only |

기본값은 `moderate`입니다. 검토마다 `"이거 엄격하게 검토해줘"` 또는 `"do a loose review"`처럼 덮어쓸 수 있습니다.
...
템플릿과 선례는 기본적으로 자동 승인되므로, 별도의 수동 승인 단계가 필요하지 않습니다.

### 검색 전략

임베딩이나 벡터 데이터베이스는 사용하지 않습니다. 검색은 다음 단계로 이루어집니다:
...
완전히 감사 가능합니다. 모든 매칭 경로를 추적할 수 있습니다.
...
- **사람이 최종 결정한다** — 에이전트는 제안하고, 인간이 판단합니다
- **로컬 및 감사 가능성** — 모든 데이터는 디스크에 있고, 모든 산출물을 직접 확인할 수 있습니다
- **대상자 방화벽** — 내부 전략은 외부 공유용 결과물로 새지 않습니다
- **재개 가능성** — 파이프라인 상태를 저장하므로 중단 후 이어서 실행할 수 있습니다
- **산업 중립성** — 도메인 특화는 코드가 아니라 정책 파일에 들어 있습니다
...
핵심 아키텍처 선택 사항:
- **임베딩 / 벡터 DB 없음** — 검색은 결정적 JSON 인덱스 필터링 + LLM 판단으로 처리합니다
- **파이프라인 상태 저장** — 각 단계가 `pipeline-state.json`을 기록하므로 중단 후 재개할 수 있습니다
- **대상자 방화벽** — `[INTERNAL]`과 `[EXTERNAL]` 코멘트 스트림을 모든 단계에서 엄격히 분리합니다
- **파일 기반 데이터 전달** — 큰 페이로드는 인라인이 아니라 `matters/` 또는 `library/runs/` 아래 파일로 주고받습니다
